...
-- | Tests for the 'Data.SplayTree.Set' module.  We test functions by
...
import qualified Data.SplayTree.Set as S
...
pFoldl' :: Int -> [Int] -> Bool
pFoldl' z0 = L.foldl' (+) z0 `eq` F.foldl' (+) z0
...
-- | Check that a function operating on a 'Set' is equivalent to
-- one operating on a 'Model'.
eq :: (Eq a, Ord a, Eq b)
   => (Model a -> b)      -- ^ Function that modifies a 'Model' in the same
                          -- way
   -> (S.Set a -> b)      -- ^ Function that modified a 'Set'
   -> [a]                 -- ^ Initial content of the 'Set' and 'Model'
...
toAscList :: Ord a => S.Set a -> [a]
toAscList = L.sort . F.toList
